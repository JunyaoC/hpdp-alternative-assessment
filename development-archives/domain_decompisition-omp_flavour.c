...
int main(){
    ...
    if (world_rank == 0){
        ...
        // We are using a Master Slave Model. #Sorry Lincoln
    ...
    if(world_rank == world_size-1){
        ...
        while(1){
            ...
            for(int i=0;i<identifiedCursor;i++){
                for(int j=0;j<identifiedCursor;j++){
                    ...
                    if(i!=j && identifiedTL_X[i] > 0){
                        if(identifiedTL_X[i] == identifiedTL_X[j] && identifiedBR_X[i] == identifiedBR_X[j]){

                            noSame = 0;

                            if(identifiedTL_Y[i] > identifiedTL_Y[j]){
                                identifiedTL_Y[i] = identifiedTL_Y[j];

                            }

                            if(identifiedBR_Y[i] < identifiedBR_Y[j]){
                                identifiedBR_Y[i] = identifiedBR_Y[j];
        ...
        for(int i=0;i<identifiedCursor;i++){
            if(identifiedTL_X[i] > 0){
                printf("Rectangle %d coordinates [  Top Left  ]: (%d,%d)\n",rectangle_count,(identifiedTL_X[i]+1),(identifiedTL_Y[i]+1));
                printf("Rectangle %d coordinates [Bottom Right]: (%d,%d)\n",rectangle_count,(identifiedBR_X[i]+1),(identifiedBR_Y[i]+1));
